...
static void print_pcie_at_scale(WINDOW *win, unsigned int value) {
  ...
  FILE* fp = fopen("/home/root/log/nvtop.log", "a+");
  fprintf(fp, "%.3f %sB/s", val_d, memory_prefix[prefix_off]);
  fclose(fp);
...
static void draw_devices(struct device_info *dev_info,
                         struct nvtop_interface *interface, char* out) {
  ...
  for (unsigned int i = 0; i < num_devices; ++i) {
    ...
    if (out != NULL)
    {
        ...
        if (IS_VALID(total_memory_valid, dinfo->valid) && IS_VALID(used_memory_valid, dinfo->valid)) 
        {
          ...
          sprintf(temp, " MEM:%.1f%s/%.1f%s\n", used_mem, memory_prefix[prefix_off], total_mem, memory_prefix[prefix_off]);
          // printf("MEM:%.1f%s/%.1f%s\n", used_mem, memory_prefix[prefix_off], total_mem, memory_prefix[prefix_off]);
        } 
        else 
        {
            sprintf(temp, " %s\n", "MEM:N/A");
        ...
        // POWER
        if (IS_VALID(power_draw_valid, dinfo->valid) && IS_VALID(power_draw_max_valid, dinfo->valid)) 
        {
            sprintf(temp, " POW:%3u/%3u W\n", dinfo->power_draw / 1000, dinfo->power_draw_max / 1000);
            // printf("POW:%3u/%3u W\n", dinfo->power_draw / 1000, dinfo->power_draw_max / 1000);
        }
        else 
        {
    ...
    if (IS_VALID(gpu_util_rate_valid, dinfo->valid)) {
      ...
      fprintf(fp, "GPU:%u%% ", dinfo->gpu_util_rate);
    } else {
    ...
    if (IS_VALID(total_memory_valid, dinfo->valid) &&
     ...
     fprintf(fp, "MEM:%.1f%s/%.1f%s ", used_mem, memory_prefix[prefix_off],
               total_mem, memory_prefix[prefix_off]);
    ...
    if (IS_VALID(gpu_temp_valid, dinfo->valid)) {
      ...
      if (!interface->use_fahrenheit)
        temp_convert = dinfo->gpu_temp;
      else
        temp_convert = (unsigned)(32 + nearbyint(dinfo->gpu_temp * 1.8));
      fprintf(fp, "TEMP:%3uC ", temp_convert);
    } else {
    ...
    if (IS_VALID(fan_speed_valid, dinfo->valid)) {
      ...
      fprintf(fp, "FAN:%3u%% ", dinfo->fan_speed);
    }
    else {
    ...
    if (IS_VALID(gpu_clock_speed_valid, dinfo->valid)) {
      ...
      fprintf(fp, "GPU:%uMHz ", dinfo->gpu_clock_speed);
    }
    else {
      mvwprintw(dev->gpu_clock_info, 0, 0, "GPU N/A MHz");
      fprintf(fp, "GPU:N/A MHz ");
    }
    ...
    if (IS_VALID(mem_clock_speed_valid, dinfo->valid)) {
      ...
      fprintf(fp, "MEM:%uMHz ", dinfo->mem_clock_speed);
    }
    else {
      mvwprintw(dev->mem_clock_info, 0, 0, "MEM N/A MHz");
      fprintf(fp, "MEM:N/A MHz ");
    }
    ...
    if (IS_VALID(power_draw_valid, dinfo->valid) &&
      ...
      mvwprintw(dev->power_info, 0, 0, "POW %3u / %3u W",
                dinfo->power_draw / 1000, dinfo->power_draw_max / 1000);
      fprintf(fp, "POW:%3u/%3u W ",
                dinfo->power_draw / 1000, dinfo->power_draw_max / 1000);
    }
    else {
      mvwprintw(dev->power_info, 0, 0, "POW N/A W");
      fprintf(fp, "POW N/A W ");
    }
    ...
    if (IS_VALID(cur_pcie_link_gen_valid, dinfo->valid) &&
      ...
      fprintf(fp, "PCIe GEN %u@%2ux ", dinfo->cur_pcie_link_gen,
              dinfo->cur_pcie_link_width);
    }
    else {
...
static void print_processes_on_screen(unsigned int num_process,
  ...
  for (unsigned int i = start_at_process; i < end_at_process && i < num_process;
    ...
    fprintf(fp, "pid:%*s user:%*s gpuid:%*s ", 
                 sizeof_process_field[process_pid], pid_str,
                 sizeof_process_field[process_user], proc[i].user_name,
                 sizeof_process_field[process_gpu_id], guid_str);
...
void draw_gpu_info_ncurses(struct device_info *dev_info,
  ...
  draw_devices(dev_info, interface, NULL);
...
{
  update_retained_data(dev_info, interface);
  draw_devices(dev_info, interface, out);
  doupdate();
  refresh();
}
